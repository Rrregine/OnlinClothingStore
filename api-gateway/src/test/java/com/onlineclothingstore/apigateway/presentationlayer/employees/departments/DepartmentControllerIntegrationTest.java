...
class DepartmentControllerIntegrationTest {
    ...
    @MockBean
    private DepartmentsServiceClient departmentServiceClient;

    @Test
    public void whenGetDepartments_thenReturnAllDepartments_PositivePath() {
        // Arrange
        List<DepartmentResponseModel> mockedDepartments = Arrays.asList(
                new DepartmentResponseModel(),
                new DepartmentResponseModel()
        );

        when(departmentServiceClient.getAllDepartments()).thenReturn(mockedDepartments);

        // Act and Assert
        webTestClient.get().uri("/api/v1/departments")
                ...
                .value((list) -> {
                    assertNotNull(list);
                    assertEquals(mockedDepartments.size(), list.size());
                });
    }
   ...
   /* @Test
    public void whenGetDepartmentByDepartmentId_thenReturnDepartment_PositivePath() {
        ...
        webTestClient.get().uri("/api/v1/departments/" + FOUND_DEPARTMENT_ID)
                .accept(MediaType.APPLICATION_JSON)
                .exchange()
                .expectStatus().isOk()
                .expectHeader().contentType(MediaType.APPLICATION_JSON)
    ...
    public void whenValidDepartment_thenUpdateDepartment_PositivePath() {
        ...
        webTestClient.put()
                ...
                .headers(headers -> log.info("Request Headers: {}", headers)) // Log request headers
                .exchange()
                .expectStatus().isOk()
                .expectHeader().contentType(MediaType.APPLICATION_JSON)
                .expectBody(DepartmentResponseModel.class)
